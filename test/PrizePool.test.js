...
describe("PrizePool", function() {
  ...
  beforeEach(async () => {
    ...
    yieldSourceStub = await deployMockContract(
      wallet,
      YieldSourceStub.abi,
      overrides
    );
    ...
    prizeStrategy = await deployMockContract(
      wallet,
    ...
    await prizeStrategy.mock.supportsInterface
      .withArgs("0xffffffff")
      .returns(false);

    const ReserveInterface = await hre.artifacts.readArtifact(
      "ReserveInterface"
    );
    reserve = await deployMockContract(wallet, ReserveInterface.abi, overrides);

    const RegistryInterface = await hre.artifacts.readArtifact(
      "RegistryInterface"
    );
    reserveRegistry = await deployMockContract(
      wallet,
      RegistryInterface.abi,
      overrides
    );
    await reserveRegistry.mock.lookup.returns(reserve.address);

    debug("deploying PrizePoolHarness...");
    const PrizePoolHarness = await hre.ethers.getContractFactory(
      "PrizePoolHarness",
      wallet,
      overrides
    );
  ...
  describe("initialize()", () => {
    it("should fire the events", async () => {
      ...
      await expect(tx)
        ...
        .withArgs(reserve.address, poolMaxExitFee, poolMaxTimelockDuration);
  ...
  describe("with a mocked prize pool", () => {
    ...
    describe("beforeTokenTransfer()", () => {
      ...
      it("should allow controlled tokens to call", async () => {
        ...
        await prizeStrategy.mock.beforeTokenTransfer
          .withArgs(wallet.address, wallet2.address, toWei("1"), ticket.address)
          .returns();
        await ticket.call(
          prizePool,
          "beforeTokenTransfer",
      ...
      it("should accrue credit to the sender and receiver", async () => {
        ...
        // trigger credit init
        await prizePool.calculateEarlyExitFee(
          wallet.address,
          ticket.address,
          42
        );
      ...
      it("should allow a user to transfer to themselves", async () => {
        ...
        await prizeStrategy.mock.beforeTokenTransfer
          .withArgs(wallet.address, wallet.address, toWei("50"), ticket.address)
          .returns();
        await ticket.call(
          prizePool,
          "beforeTokenTransfer",
    ...
    describe("initialize()", () => {
      it("should set all the vars", async () => {
        ...
        expect(await prizePool.reserveRegistry()).to.equal(
          reserveRegistry.address
        );
      });
    ...
    describe("depositTo()", () => {
      it("should mint timelock tokens to the user", async () => {
        ...
        await erc20token.mock.transferFrom
          .withArgs(wallet.address, prizePool.address, amount)
          .returns(true);
        await yieldSourceStub.mock.supply.withArgs(amount).returns();
        await prizeStrategy.mock.beforeTokenMint
          .withArgs(wallet2.address, amount, ticket.address, AddressZero)
          .returns();
        await ticket.mock.controllerMint
          .withArgs(wallet2.address, amount)
          .returns();
    ...
    describe("timelockDepositTo", () => {
      it("should revert when deposit exceeds liquidity cap", async () => {
        ...
        await expect(
          prizePool.timelockDepositTo(wallet2.address, amount, ticket.address)
        ).to.be.revertedWith("PrizePool/exceeds-liquidity-cap");
      });
    });
    ...
    describe("calculateReserveFee()", () => {
      it("should return zero when no reserve fee is set", async () => {
        ...
        expect(await prizePool.calculateReserveFee(toWei("1"))).to.equal(
          toWei("0")
        );
      });
    ...
    describe("withdrawReserve()", () => {
      it("should allow the reserve to be withdrawn", async () => {
        ...
        await erc20token.mock.transfer
          .withArgs(wallet.address, toWei("0.8"))
          .returns(true);

        // capture the reserve of 1 token
        await prizePool.captureAwardBalance();

        await yieldSourceStub.mock.redeem
          .withArgs(toWei("1"))
          .returns(toWei("0.8"));
    ...
    describe("calculateEarlyExitFee", () => {
      it("should return the early exit for for a withdrawal", async () => {
        ...
        ).to.deep.equal([toWei("0.5"), toWei("0.5")]);
    ...
    describe("withdrawInstantlyFrom()", () => {
      it("should allow a user to withdraw instantly", async () => {
        ...
        await yieldSourceStub.mock.redeem
          ...
          .returns(toWei("9"));
        await erc20token.mock.transfer
          .withArgs(wallet.address, toWei("9"))
          .returns(true);
      ...
      it("should only transfer to the user the amount that was redeemed", async () => {
        ...
        await yieldSourceStub.mock.redeem
          ...
          .returns(redeemed);
        await erc20token.mock.transfer
          .withArgs(wallet.address, redeemed)
          .returns(true);
      ...
      it("should allow a user to set a maximum exit fee", async () => {
        ...
        await ticket.mock.controllerBurnFrom
          .withArgs(wallet2.address, wallet.address, amount)
          .returns();
        ...
        await expect(
          prizePool
            .connect(wallet2)
            .withdrawInstantlyFrom(wallet.address, amount, ticket.address, fee)
        )
          .to.emit(prizePool, "InstantWithdrawal")
          .withArgs(
            wallet2.address,
    ...
    describe("withdrawWithTimelockFrom()", () => {
      it("should allow a user to withdraw with a timelock", async () => {
        ...
        await prizePool.withdrawWithTimelockFrom(
          wallet.address,
          amount,
          ticket.address
        );

        expect(await prizePool.timelockBalanceOf(wallet.address)).to.equal(
          amount
        );
        expect(
          await prizePool.timelockBalanceAvailableAt(wallet.address)
        ).to.equal(11);
      ...
      it("should limit the duration of the timelock", async () => {
        ...
        expect(
          await prizePool.timelockBalanceAvailableAt(wallet.address)
        ).to.equal(1 + poolMaxTimelockDuration); // current time + 10000
        expect(await prizePool.timelockTotalSupply()).to.equal(amount);
      });
    });
    ...
    describe("sweepTimelockBalances()", () => {
      ...
      it("should sweep only balances that are unlocked", async () => {
        ...
        await erc20token.mock.transfer
          .withArgs(wallet.address, amount1)
          .returns(true);

        // Let's sweep
        await expect(
          prizePool.sweepTimelockBalances([wallet.address, wallet2.address])
        )
        ...
        // second has not
        expect(await prizePool.timelockBalanceOf(wallet2.address)).to.equal(
          amount2
        );
        expect(
          await prizePool.timelockBalanceAvailableAt(wallet2.address)
        ).to.equal(21);
      ...
      it("should sweep timelock balances that have unlocked", async () => {
        ...
        await ticket.mock.controllerBurnFrom
          ...
          .returns();
        await ticket.mock.controllerBurnFrom
          .withArgs(wallet2.address, wallet2.address, amount2)
          .returns();
        ...
        await prizePool
          .connect(wallet2)
          .withdrawWithTimelockFrom(wallet2.address, amount2, ticket.address);
        ...
        await yieldSourceStub.mock.redeem
          ...
          .returns(toWei("20"));
        await erc20token.mock.transfer
          .withArgs(wallet.address, toWei("5"))
          .returns(true);
        await erc20token.mock.transfer
          .withArgs(wallet2.address, toWei("15"))
          .returns(true);
        ...
        let tx = prizePool.sweepTimelockBalances([
          wallet.address,
          wallet2.address,
        ]);
    ...
    describe("calculateTimelockDuration()", () => {
      it("should return the timelock duration", async () => {
        ...
        await prizePool.calculateTimelockDuration(
          wallet.address,
          ticket.address,
          amount
        );
        ...
        ).to.deep.equal([ethers.BigNumber.from("5"), toWei("0.5")]);
    ...
    describe("balance()", () => {
      it("should return zero if no deposits have been made", async () => {
        ...
        expect((await call(prizePool, "balance")).toString()).to.equal(
          toWei("11")
        );
      });
    });
    ...
    describe("setPrizeStrategy()", () => {
      ...
      it("should not allow anyone else to change the prize strategy", async () => {
        await expect(
          prizePool.connect(wallet2).setPrizeStrategy(wallet2.address)
    ...
    describe("setLiquidityCap", () => {
      ...
      it("should not allow anyone else to call", async () => {
        ...
        await expect(
          prizePool2.setLiquidityCap(toWei("1000"))
        ).to.be.revertedWith("Ownable: caller is not the owner");
      });
    });
    ...
    describe("compLikeDelegate()", () => {
      it("should delegate votes", async () => {
        ...
        await compLike.mock.delegate
          .withArgs(wallet2.address)
          .revertsWithReason("hello");
        await expect(
      ...
      it("should only allow the owner to delegate", async () => {
        await expect(
          prizePool
            .connect(wallet2)
            .compLikeDelegate(compLike.address, wallet2.address)
        ).to.be.revertedWith("Ownable: caller is not the owner");
      });
  ...
  describe("with a multi-token prize pool", () => {
    beforeEach(async () => {
      ...
      multiTokenPrizeStrategy = await deployMockContract(
        wallet,
        TokenListenerInterface.abi,
        overrides
      );
      await multiTokenPrizeStrategy.mock.supportsInterface.returns(true);
      await multiTokenPrizeStrategy.mock.supportsInterface
        .withArgs("0xffffffff")
        .returns(false);
      ...
      await multiTokenPrizePool.setPrizeStrategy(
        multiTokenPrizeStrategy.address
      );
      await multiTokenPrizePool.setCreditPlanOf(
        ticket.address,
        toWei("0.01"),
        toWei("0.1")
      );
    ...
    describe("accountedBalance()", () => {
      it("should return the total accounted balance for all tokens including timelocked deposits", async () => {
        ...
        expect(await multiTokenPrizePool.accountedBalance()).to.equal(
          toWei("1368")
        );
      });
  ...
  describe("transferExternalERC20()", () => {
    ...
    it("should only allow the prizeStrategy to award external ERC20s", async () => {
      ...
      let prizePool2 = prizePool.connect(wallet2);
      await expect(
        prizePool2.transferExternalERC20(
          wallet.address,
          wallet2.address,
          toWei("10")
        )
  ...
  describe("awardExternalERC721()", () => {
    ...
    it("should only allow the prizeStrategy to award external ERC721s", async () => {
      ...
      let prizePool2 = prizePool.connect(wallet2);
      await expect(
        prizePool2.awardExternalERC721(wallet.address, erc721token.address, [
          NFT_TOKEN_ID,
        ])
      ).to.be.revertedWith("PrizePool/only-prizeStrategy");
    });
    ...
    it("should allow arbitrary tokens to be transferred", async () => {
      ...
      await erc721token.mock.transferFrom
        .withArgs(prizePool.address, wallet.address, NFT_TOKEN_ID)
        .returns();
      await expect(
        prizeStrategy.call(
          prizePool,

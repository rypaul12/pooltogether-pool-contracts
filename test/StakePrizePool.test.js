...
describe('StakePrizePool', function() {
  ...
  describe('initialize()', () => {
    ...
    it('should fail to initialize StakePrizePool if stakeToken is not a contract', async () => {
      await expect(
        initializeStakePrizePool(ethers.Wallet.createRandom().address),
      ).to.be.revertedWith(prizePool, 'StakePrizePool/stake-token-not-contract-address');
    })
